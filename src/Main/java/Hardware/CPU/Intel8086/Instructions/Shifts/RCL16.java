...
public final class RCL16 extends Instruction {

    private final Operand m_destination;
    private final Operand m_count;
    
    public RCL16(Intel8086 cpu,
                 Operand destination,
                 Operand count,
                 int cycles) {
    ...
    public void run() {
        ...
        int result = (dest << (count % 17)) | (dest >>> (17 - (count % 17)));
        
        if(count == 1)
            m_cpu.FLAGS.OF = ((dest ^ result) & 0x8000) != 0;
        
        m_cpu.FLAGS.CF = (result & 0x10000) != 0;
    ...
    public String toString() {
        ...
        return String.format("rcl %s, %s", m_destination.toString(), m_count.toString());
    }
}
