...
public final class DEC16 extends Instruction {

    private final Operand m_destination;

    public DEC16(Intel8086 cpu,
                 Operand destination,
                 int cycles) {
    ...
    public void run() {
        ...
        int result = (m_destination.getValue() - 1) & 0xffff;
        
        m_cpu.FLAGS.setSZP_16(result);
        m_cpu.FLAGS.OF = result == 0x7fff;
        m_cpu.FLAGS.AF = (result & 0x0f) == 0x0f;
    ...
    public String toString() {
        ...
        return String.format("dec %s", m_destination.toString());
    }
}
