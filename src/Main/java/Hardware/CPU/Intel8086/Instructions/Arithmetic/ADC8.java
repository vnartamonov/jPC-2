...
public final class ADC8 extends Instruction {

    private final Operand m_destination;
    private final Operand m_source;
    
    public ADC8(Intel8086 cpu,
                Operand destination,
                Operand source,
                int cycles) {
    ...
    public void run() {
        ...
        int result = (dest + src + (m_cpu.FLAGS.CF ? 1 : 0)) & 0xff;
        
        m_cpu.FLAGS.setSZP_8(result);
        m_cpu.FLAGS.CF = (Integer.compareUnsigned(result, dest) < 0) || (m_cpu.FLAGS.CF && (result == dest));
        m_cpu.FLAGS.OF = (((dest ^ src ^ 0x80) & (result ^ src)) & 0x80) != 0;
    ...
    public String toString() {
        ...
        return String.format("adc %s, %s", m_destination.toString(), m_source.toString());
    }
}
