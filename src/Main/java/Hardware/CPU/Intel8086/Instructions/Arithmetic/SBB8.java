...
public final class SBB8 extends Instruction {

    private final Operand m_destination;
    private final Operand m_source;
    
    public SBB8(Intel8086 cpu,
                Operand destination,
                Operand source,
                int cycles) {
    ...
    public void run() {
        ...
        m_cpu.FLAGS.CF = (Integer.compareUnsigned(dest, result) < 0) || (m_cpu.FLAGS.CF && (src == 0xff));
        m_cpu.FLAGS.OF = (((dest ^ src) & (dest ^ result)) & 0x80) != 0;
